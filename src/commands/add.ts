...
export async function runAdd(store: VaultStore, pass: string, args: string[]): Promise<number> {
  const key = args[0];
  if (!key) {
    console.log('⚠️ Specify an entry name');
    return 1;
  }
  if (store[key]) {
  ...
  const password = (await ask('Password (leave blank to generate): ')) || generatePassword();
